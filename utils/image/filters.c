	...
	for (int y = 0; y < new->h; y++) {
		for (int x = 0; x < new->w; x++) {
			...
			if ((color.r + color.g + color.b) / 3 > threshold)
				...
				newColor = (SDL_Color){.r = 0, .g = 0, .b = 0};

			image_putPixel(new, x, y,
				       SDL_MapRGBA(new->format, newColor.r,
...
SDL_Surface *filter_createGroup(SDL_Surface *s)
{
	...
	for (int y = 0; y < s->h; y++) {
		for (int x = 0; x < s->w; x++) {
			SDL_Color color =
			    _filter_createGroup_work(s, 25, 1, x, y);

			image_putPixel(new, x, y,
				       SDL_MapRGBA(new->format, color.r,
						   color.g, color.b, 255));
		}
	}
...
SDL_Color _filter_createGroup_work(SDL_Surface *s, unsigned matrixSize,
				   unsigned threshold, unsigned x, unsigned y)
{
	unsigned nbAround = 0;
	...
	SDL_Color finalcolor = {.r = (nbAround >= threshold) ? 0 : 255,
