...
struct charRecognition *charRecognition_learn(char *rootPath, char chars[],
					      size_t size, size_t variants)
{
	...
	for (unsigned i = 0; i < size; i++) {
		for (unsigned j = 0; j < variants; j++) {
			...
			SDL_Surface *s =
			    image_scale(image_crop(image_load(path)), 16, 16);
			...
			for (unsigned k = 0; k < size; k++)
				output.elements[count].elements[k] =
				    ((count / variants) == k);
	...
	NeuralNetwork_train(myNeuralNetwork, input, output, 0.05, 0.1, 0.9);
...
struct charRecognition *charRecognition_learnWeights(char *pathIn,
						     char *pathOut,
						     char chars[], size_t size)
{
	...
	NeuralNetwork_loadWeightInput(myNeuralNetwork,
				      file_get_content(pathIn));
	NeuralNetwork_loadWeightOutput(myNeuralNetwork,
				       file_get_content(pathOut));
			     ...
			     SDL_Surface *surface)
{
	...
	doubleArray ratio = NeuralNetwork_testDouble(charReg->network, input);
